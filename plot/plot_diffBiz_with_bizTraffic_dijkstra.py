...
PLOT_OUTPUT = r"F:\HeteroSat_Routing_Sim\plot\Load_Sensitivity_DiffBiz_Traffic_Dijkstra.png"
# ==========================================
...
def plot_biz_sensitivity(df, save_path):
    ...
    metrics = [
        ("AvgGoodput", "Avg Goodput per Flow (Mbps)", "(a) Average Goodput vs Traffic Volume (Dijkstra)"),
        ("AvgDelay", "Average Delay (ms)", "(b) Average End-to-End Delay vs Traffic Volume (Dijkstra)"),
        ("AvgLoss", "Average Loss (%)", "(c) Average Packet Loss vs Traffic Volume (Dijkstra)")
    ]
...
def main():
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    csv_out = os.path.join(out_dir, "Dijkstra_BizQoS_vs_Traffic.csv")
