...
PLOT_OUTPUT = r"F:\HeteroSat_Routing_Sim\plot\Load_Sensitivity_DiffBiz_Traffic_SGA.png"
# ==========================================
...
def plot_biz_sensitivity(df, save_path):
    ...
    metrics = [
        ("AvgGoodput", "Avg Goodput per Flow (Mbps)", "(a) Average Goodput vs Traffic Volume (SGA)"),
        ("AvgDelay", "Average Delay (ms)", "(b) Average End-to-End Delay vs Traffic Volume (SGA)"),
        ("AvgLoss", "Average Loss (%)", "(c) Average Packet Loss vs Traffic Volume (SGA)")
    ]
...
def main():
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    csv_out = os.path.join(out_dir, "SGA_BizQoS_vs_Traffic.csv")
